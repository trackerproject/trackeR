...
smoothed distribution profiles for the 26~sessions. The corresponding
concentration profiles are shown in Figure~\ref{fig:app:cpsmooth}.
%
...
components is fitted and the share of variance displayed.
% 
<<AppFunPrep>>=
library("fda")
