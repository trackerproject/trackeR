...
\Abstract{This introduction to the \proglang{R} package \pkg{trackeR} is a (slightly)
  modified version of \cite{trackeR:Frick+Kosmidis:2017}, published in the
...
how often, much, or hard an individual or a group trains, and a more advanced 
outlook tries to explain the impact of training on athlete physiology or 
performance.
...
%% %% Note: the successor of cycleRtools, elpatron, is currently not on CRAN
%% %% anymore due to license issues.
...
focus on basic methods for unit manipulation as well as smoothing and
thresholding. The case study in Section~\ref{sec:casestudy}
investigates the key features in \Sexpr{nsessions(runs)} sessions through a
...
\begin{figure}[t]
  ...
  {\scriptsize %\tiny %\footnotesize
    %\input{pkg_structure_revision_read} %% NOTE: \setlength{\du}{10\unitlength} was changed manually.
% Graphic for TeX using PGF
% Title: /home/frick/svn/EISproject/pkg/trackeR/vignettes/pkg_structure_revision_read.dia
% Creator: Dia v0.97.3
% CreationDate: Wed Oct 12 20:25:09 2016
% For: frick
% \usepackage{tikz}
...
\begin{figure}[t]
  \centering
  {\scriptsize %\tiny %\footnotesize
  % \input{pkg_structure_revision_rest} %% NOTE: \setlength{\du}{10\unitlength} was changed manually.
% Graphic for TeX using PGF
% Title: /home/frick/svn/EISproject/pkg/trackeR/vignettes/pkg_structure_revision_rest.dia
% Creator: Dia v0.97.3
% CreationDate: Wed Oct 12 20:26:29 2016
% For: frick
% \usepackage{tikz}
% The following commands are not supported in PSTricks at present
% We define them conditionally, so when they are implemented,
...
Data in various formats are imported and stored in the central data object of \linebreak
...
A list of all functionality is provided in Tables~\ref{tab:pkg:functions:1} and~\ref{tab:pkg:functions:2}.
...
\begin{table}[p]
  ...
  \begin{tabular}{l l p{6.5cm}}
    ...
    \fct{readContainer} & TCX/DB3/JSON file & read a TCX/DB3/JSON file \\
    \fct{readDirectory} & TCX/DB3/JSON files & read all TCX/DB3/JSON files in a
    \linebreak directory \\
    ...
    \fct{smoother} & \class{trackeRdata} & smooth data by applying a summary
    ...
    \fct{nsessions} & \class{trackeRdata} & number of sessions \\
    ...
    \fct{nsessions} & \class{trackeRdataZones} & number of sessions \\
    ...
    \fct{plot} & \class{trackeRWprime} & plot \emph{$W'$ balance} or \emph{$W'$
      expended} \\
    \fct{nsessions} & \class{trackeRWprime} & number of sessions \\
    %
    \hline
  \end{tabular}
...
\begin{table}[t]
  ...
  \begin{tabular}{l l p{7cm}}
    ...
    \fct{nsessions} & \class{distrProfile} & number of sessions \\
    ...
    \fct{profile2fd} & \class{distrProfile} & convert profiles to \class{fd} class \\
    \fct{funPCA} & \class{distrProfile} & functional principal components analysis \\
    %
    ...
    \fct{nsessions} & \class{conProfile} & number of sessions \\
    ...
    \fct{profile2fd} & \class{conProfile} & convert profiles to \class{fd} class \\
    \fct{funPCA} & \class{conProfile} & functional principal components analysis \\
    \hline
  \end{tabular}
...
\pkg{trackeR} provides utilities for data in common
...
reading TCX (Training Centre XML), DB3 (for SQLite, used, e.g., by devices
from GPSports) and Golden Cheetah's JSON files. These functions read the
...
% @
%
  \includegraphics[page=2]{plotRouteGoogle}
...
The following code chunk displays the current units of \code{run}, changes the
unit for speed to miles per hour, and displays the changed units.
%
<<eval = TRUE>>=
getUnits(run)
...
%
%
<<AppProfiles0, eval = FALSE, echo = FALSE>>=
library("trackeR")
data("runs", package = "trackeR")
runsT <- threshold(runs)
dpRuns <- distributionProfile(runsT, what = "speed")
