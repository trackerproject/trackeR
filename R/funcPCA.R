...
profile2fd <- function(object, what, ...){
    ...
    if (!(what %in% names(object))) stop(paste("Profiles for", what, "not found."))
...
plot.trackeRfpca <- function(x, harm = NULL, expand = NULL, pointplot = TRUE, ...){
    ## function(x, nx = 128, pointplot = TRUE, harm = 0,
    ##                     expand = 0, cycle = FALSE, ...)
    #argvals <- seq(x[["harmonics"]]$basis$rangeval[1], x[["harmonics"]]$basis$rangeval[2], length = 128)
    ## harm <- NULL
    ## expand <- NULL
    ...
    if (is.null(harm)) harm <- seq_len(length(x[["varprop"]]))
    nharm <- length(harm)

    ## combine to data.frame in long format for plotting
    df <- data.frame("argval" = rep(argvals, times = nharm),
                     "meanval" = rep(meanMat, times = nharm),
                     "pc" = rep(harm, each = length(argvals)),
                     "pcval" = as.vector(harmonicsMat[,harm]))

    ## add suitable multitude of the harmonics to the mean function
    if (is.null(expand)) expand <- sqrt(x[["values"]][harm])
    ...
    p <- ggplot2::ggplot(data = df) +
        ggplot2::labs(x = lab_data(attr(x, "what")),
                      y = if (attr(x, "profile") == "distrProfile") "time above threshold" else "d time")
    ...
    if (pointplot) {
        p <- p + ggplot2::geom_point(ggplot2::aes_(x = quote(argval), y = quote(meanPlusPC)), pch = "+") +
            ggplot2::geom_point(ggplot2::aes_(x = quote(argval), y = quote(meanMinusPC)), pch = "-")
    } else {
        p <- p + ggplot2::geom_line(ggplot2::aes_(x = quote(argval), y = quote(meanPlusPC)), lty = 2) +
            ggplot2::geom_line(ggplot2::aes_(x = quote(argval), y = quote(meanMinusPC)), lty = 3)
    }

    ## facets for harmonics
    ## create lookup table
    lab_pc <- function(pc){
